...
Last updated: 2026-09-12 (Phase 2A.5 QA: editor overflow, unpublish, preview labels)
...
| Logo upload       | Application path complete: ADMIN upload/replace/remove, PNG/JPEG/WebP + sanitized SVG, same-origin `/clinic-branding/...` GET, `nosniff` + CSP. **External `clinic-branding` bucket still required** for production. Memory driver is tests-only. See [CLINIC-ASSETS.md](../architecture/CLINIC-ASSETS.md).                                                                                                                                                                |
...
Provision the private Supabase `clinic-branding` bucket and set `CLINIC_ASSET_STORAGE_DRIVER=supabase` plus the service-role key. Run `scripts/provision-clinic-branding-bucket.mjs` locally — do not execute it from Cursor against production.
...
| Area           | Behaviour                                                                                                                                                                                                                                                                                                                                                                                                    |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Portal shell   | Desktop contract: shell `width 100%` / `min-width 0` / `100dvh` / `overflow: hidden`; sidebar `flex: 0 0 16rem`; `staffAppMain` `flex: 1 1 0` + `min-width 0`; scrolling `main.staffAppScroller` is the primary landmark and `overflow-y: auto` / `overflow-x: clip`. Content wrapper owns padding and `min-width: 0`. No `100vw` in staff chrome. No global `overflow-x: hidden`.                           |
...
| Overflow tests | `e2e/helpers/portal-shell.ts` fails if `scrollWidth > clientWidth + 1` on document, `.staffAppMain`, `.staffAppScroller`, `.staffAppContent`, and `.staffEditorPage` when present. Covers Overview, Guides, editor, Practice, Operator All Clinics, operator clinic detail, plus 1100/900 and editor resize.                                                                                                 |
...
| Future search  | Do **not** add a schema field this pass. Default remains private-from-search. A later `searchVisibility` (or equivalent) may let a clinic opt a published guide into indexing, with operator policy above it. SEO fields belong on Guide Template defaults, clinic Guide overrides, and Practice defaults — **not** a standalone Operator SEO tab.                                                           |
| Operator CMS   | Operator is the future content/control plane (Clinics, then Templates / content management). Do not add empty Templates or SEO navigation.                                                                                                                                                                                                                                                                   |
...
| Preview theme    | Seed/clinic default is `ClinicProfile.themeMode = SYSTEM`. Dark patient preview under portal Light is expected when the OS prefers dark. Preview label is `Clinic default (System/Light/Dark)`. Override does not persist Practice.                                                                                                                                |
